# `ph_openshift_autoshift.automation`
...
2. Edit `galaxy.yml` — replace `<your_namespace>`, `ph_openshift_autoshift.automation`, and
   the author line. These become the prefix for every role's fully qualified name
   (`<your_namespace>.ph_openshift_autoshift.automation.<role_name>`).
...
- name: Run my_role_name
  ansible.builtin.include_role:
    name: <your_namespace>.ph_openshift_autoshift.automation.my_role_name
```
